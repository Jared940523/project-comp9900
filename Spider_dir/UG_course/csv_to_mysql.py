...
cur = conn.cursor()
drop_table = '''DROP TABLE IF EXISTS ug_course_info'''
cur.execute(drop_table)

create_table = ''' 
create table ug_course_info(
...
drop_table = '''DROP TABLE IF EXISTS ug_course_desp'''
cur.execute(drop_table)
create_table = '''
create table ug_course_desp(
...
drop_table = '''DROP TABLE IF EXISTS ug_faculty'''
cur.execute(drop_table)
create_table = '''
create table ug_faculty(
faculty TEXT,
url TEXT
);
'''
...
drop_table = '''DROP TABLE IF EXISTS ug_school'''
cur.execute(drop_table)
create_table = '''
create table ug_school(
school TEXT ,
url TEXT
);
'''
...
for file in csv_file:
    ...
    for item in data.values:
        ...
        sql = '''
        insert into ug_course_info (prerequisite, course_code, course_title, course_url, credit, faculty,
        ...
        sql = '''insert into ug_course_desp (course_code, description)
        VALUES ("%s", "%s")''' % (item[5], item[7])
        cur.execute(sql)

for key, value in faculty_d.items():
    sql = '''insert into ug_faculty (faculty, url) VALUES ("%s", "%s")''' % (key, value)
    cur.execute(sql)

for key, value in school_d.items():
    sql = '''insert into ug_school (school, url) VALUES ("%s", "%s")''' % (key, value)
    cur.execute(sql)
