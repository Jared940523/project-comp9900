# -*- coding: utf-8 -*-
import scrapy
from UG_course.items import *


class UgCourseInfoSpider(scrapy.Spider):
    name = 'ug_course_info'
    allowed_domains = ['legacy.handbook.unsw.edu.au']
    start_urls = ['http://legacy.handbook.unsw.edu.au/vbook2018/brCoursesByAtoZ.jsp?StudyLevel=Undergraduate&descr=All']

    def parse(self, response):
        course_info = response.xpath("//*[@id='contentPane']/div[2]/div/table/tr")

        for item in course_info:
            course_info_detail = UgCourseItem()
    ...
    def detail_parse(self, response):
        ...
        for index, i in enumerate(course_detail):
            ...
            if name == "Faculty:":
                ...
                if '"' in item['Prerequisite']:
                    item['Prerequisite'] = item['Prerequisite'].replace('"', '')
